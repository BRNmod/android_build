...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^brn_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^brn_//g')
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/brn/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/brn/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
